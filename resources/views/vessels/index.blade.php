        ...
        <div class="inner-body">
                ...
                <div class="container">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('vessels.create') }}" class="btn btn-primary d-flex align-items-center">
            ...
            <div class="card">
                <div class="card-body text-center py-4">
                    <div class="text-muted">
                        ...
                        <p class="mb-0">No vessels found</p>
                    </div>
                </div>
            </div>
            @else
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="search-container">
                            <input type="text" id="vessel-search" class="form-control" placeholder="Search by name or nationality..." value="{{ request('search') }}">
                        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0" id="vessels-table">
                            <thead>
                                <tr>
                                    <th>Name</th>
                                    <th>Actions</th>
                            ...
                            <tbody id="vessels-table-body">
                                @include('vessels.partials.table')
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <div class="d-flex justify-content-between align-items-center mt-4">
                <div class="text-muted small" id="pagination-info">
                    Showing {{ $vessels->firstItem() ?? 0 }} to {{ $vessels->lastItem() ?? 0 }} of {{ $vessels->total() }} entries
                </div>
                <div id="pagination-links">
                    {{ $vessels->appends(request()->all())->links('pagination::bootstrap-5') }}
                </div>
            </div>
            @endif
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to fetch vessels via AJAX
    function fetchVessels(search = '', perPage = 10, page = 1) {
        $.ajax({
            ...
            success: function(response) {
                // Update table body
                $('#vessels-table-body').html(response.table);
            ...
            error: function(xhr) {
                ...
                alert('An error occurred while fetching vessels. Please try again.');
            }
        });
    }
    ...
    function attachDeleteHandlers() {
        document.querySelectorAll('.delete-vessel-form').forEach(form => {
            form.addEventListener('submit', function(e) {
                e.preventDefault();
                const vesselName = this.querySelector('.delete-btn').dataset.vesselName;
    ...
    const debouncedSearch = debounce(function() {
        ...
        fetchVessels(searchTerm, perPage);
    }, 300);
    ...
    document.getElementById('per-page').addEventListener('change', function() {
        ...
        const searchTerm = document.getElementById('vessel-search').value;
        fetchVessels(searchTerm, perPage);
    });
    ...
    document.getElementById('pagination-links').addEventListener('click', function(e) {
        ...
        if (e.target.tagName === 'A' && e.target.getAttribute('href')) {
            ...
            const searchTerm = document.getElementById('vessel-search').value;
            const perPage = document.getElementById('per-page').value;
            fetchVessels(searchTerm, perPage, page);
        }
    });
